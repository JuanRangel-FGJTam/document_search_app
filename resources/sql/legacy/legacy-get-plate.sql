SELECT O.ID_OBJETO,
    ...
    H.ID_CALLE,
    O.DESCRIPCION,
    MUNICIPIO = CASE WHEN HCP.ID_LUGAR_HECHOS_CP IS NULL THEN IsNull(CM.MUNICIPIO, '') ELSE HCP.CPmunicipio END,
    LOCALIDAD = CASE WHEN HCP.ID_LUGAR_HECHOS_CP IS NULL THEN IsNull(CL.LCLDD,'') ELSE HCP.CPmunicipio END,
    COLONIA = CASE WHEN HCP.ID_LUGAR_HECHOS_CP IS NULL THEN IsNull(CC.CLNIA,'') ELSE HCP.CPcolonia END,
    CALLE = CASE WHEN HCP.ID_LUGAR_HECHOS_CP IS NULL THEN IsNull(CCA.CLLE,'') ELSE HCP.CPcalle END
...
Left Outer Join [dbo].[PGJ_HECHOS_CP] HCP on HCP.ID_EXTRAVIO = E.ID_EXTRAVIO
